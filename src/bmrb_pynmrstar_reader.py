    ...
    def run(self):
        ...
        for file in files:
            ...
            if not name in self.tables['metabolites']['name']:
                ...
                smiles_table = self.get_loop_tables(entry, 'Chem_comp_SMILES')[0]
            ...
            if len(sample_condition_tables) > 1:
                print(f'more than one sample condition in entry {entry}')
            experiment_tables = self.get_loop_tables(entry, 'Experiment')
            print(experiment_tables)
            if len(experiment_tables) > 1:
                print(f'more than one experiment conditions table in entry {entry}')
